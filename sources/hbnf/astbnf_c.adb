...
package body ASTBNF_C is
   ...
   begin
      Put ("/* ------------------------------------------------------------------ */");
      Put ("/* Lexer: text -> token stream (schema-independent).  Skips whitespace");
      Put ("/* and `#` comments; yields word/string/number/punctuation tokens.     */");
      Put ("/* ------------------------------------------------------------------ */");
      Put ("");
      Put ("typedef struct { token_t *toks; size_t n; } lexed_t;");
      Put ("");
      Put ("static int lex_digit(char c) { return c >= '0' && c <= '9'; }");
      Put ("static int lex_word_start(char c) {");
      Put ("    return (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')");
      Put ("        || c == '_' || c == '-';");
      Put ("}");
      Put ("static int lex_word_char(char c) {");
      Put ("    return lex_word_start(c) || lex_digit(c) || c == '.';");
      Put ("}");
      Put ("static char *lex_dup(const char *s, size_t n) {");
      Put ("    char *p = (char *)malloc(n + 1);");
      Put ("    if (p) { memcpy(p, s, n); p[n] = '\0'; }");
      Put ("    return p;");
      Put ("}");
      Put ("");
      Put ("lexed_t lex(const char *text) {");
      Put ("    lexed_t r = {0};");
      Put ("    token_t *toks = (token_t *)malloc((strlen(text) + 2) * sizeof *toks);");
      Put ("    size_t i = 0, line = 1, col = 1;");
      Put ("");
      Put ("    while (text[i]) {");
      Put ("        char c = text[i];");
      Put ("        if (c == ' ' || c == '\t' || c == '\r') { i++; col++; }");
      Put ("        else if (c == '\n') { i++; line++; col = 1; }");
      Put ("        else if (c == '#') { while (text[i] && text[i] != '\n') i++; }");
      Put ("        else if (c == '""') {");
      Put ("            size_t sc = col;");
      Put ("            char *buf = (char *)malloc(strlen(text + i) + 1);");
      Put ("            size_t bn = 0;");
      Put ("            i++; col++;");
      Put ("            while (text[i] && text[i] != '""') {");
      Put ("                if (text[i] == '\\' && text[i + 1]) { i++; col++; }");
      Put ("                buf[bn++] = text[i++]; col++;");
      Put ("            }");
      Put ("            if (text[i] == '""') { i++; col++; }");
      Put ("            buf[bn] = '\0';");
      Put ("            toks[r.n++] = (token_t){ TOK_STR, buf, line, sc };");
      Put ("        }");
      Put ("        else if (lex_digit(c)) {");
      Put ("            size_t s = i, sc = col; int dec = 0;");
      Put ("            while (lex_digit(text[i])) { i++; col++; }");
      Put ("            if (text[i] == '.' && lex_digit(text[i + 1])) {");
      Put ("                dec = 1; i++; col++;");
      Put ("                while (lex_digit(text[i])) { i++; col++; }");
      Put ("            }");
      Put ("            if (text[i] == 'e' || text[i] == 'E') {");
      Put ("                dec = 1; i++; col++;");
      Put ("                if (text[i] == '+' || text[i] == '-') { i++; col++; }");
      Put ("                while (lex_digit(text[i])) { i++; col++; }");
      Put ("            }");
      Put ("            toks[r.n++] = (token_t){ dec ? TOK_DEC : TOK_INT,");
      Put ("                                     lex_dup(text + s, i - s), line, sc };");
      Put ("        }");
      Put ("        else if (lex_word_start(c)) {");
      Put ("            size_t s = i, sc = col;");
      Put ("            while (lex_word_char(text[i])) { i++; col++; }");
      Put ("            toks[r.n++] = (token_t){ TOK_ATOM, lex_dup(text + s, i - s), line, sc };");
      Put ("        }");
      Put ("        else {");
      Put ("            toks[r.n++] = (token_t){ TOK_PUNCT, lex_dup(text + i, 1), line, col };");
      Put ("            i++; col++;");
      Put ("        }");
      Put ("    }");
      Put ("    toks[r.n++] = (token_t){ TOK_EOF, lex_dup("""", 0), line, col };");
      Put ("    r.toks = toks;");
      Put ("    return r;");
      Put ("}");
      Put ("");
      Put ("/* Convenience: lex, split text into lines (for the caret), then parse. */");
      Put ("bool parse_text(const char *text, " & Root_T & " *out,");
      Put ("                char *err, size_t errlen, size_t *err_line, size_t *err_col) {");
      Put ("    lexed_t l = lex(text);");
      Put ("    size_t nlines = 1, i, k = 0, s = 0;");
      Put ("    const char *p;");
      Put ("    for (p = text; *p; p++) if (*p == '\n') nlines++;");
      Put ("    char **lines = (char **)malloc(nlines * sizeof *lines);");
      Put ("    for (i = 0; ; i++) {");
      Put ("        if (text[i] == '\n' || text[i] == '\0') {");
      Put ("            lines[k++] = lex_dup(text + s, i - s);");
      Put ("            if (text[i] == '\0') break;");
      Put ("            s = i + 1;");
      Put ("        }");
      Put ("    }");
      Put ("    bool ok = parse_config(l.toks, l.n, out, (const char *const *)lines, nlines,");
      Put ("                           err, errlen, err_line, err_col);");
      Put ("    for (i = 0; i < l.n; i++) free((char *)l.toks[i].text);");
      Put ("    free(l.toks);");
      Put ("    for (i = 0; i < nlines; i++) free(lines[i]);");
      Put ("    free(lines);");
      Put ("    return ok;");
      Put ("}");
      return To_String (B);
